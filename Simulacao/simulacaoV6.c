...
void roundRobin(Base *base, int qtdEnt, int quantum)
{
  Fila fila[qtdEnt];
  ...
  {
    fila[i].ent = base[i].ent;
    fila[i].tcf = base[i].tbc;
    fila[i].tef = 0;
    fila[i].tsf = 0;
    atendente[i].ent = base[i].ent;
    atendente[i].ti = 0;
    atendente[i].tt = 0;
  }
  ...
  for (int i = 0; i < qtdEnt; i++)
  {
    restante[i] = base[i].ta;
  }

  int processosConcluidos = 0;

  while (processosConcluidos < qtdEnt)
  {
    for (int i = 0; i < qtdEnt; i++)
    {
      ...
      if (restante[i] > 0)
      {
        ...
        if (atendente[i].ti == 0)
        {
          atendente[i].ti = (tempoAtual > base[i].tbc) ? tempoAtual : base[i].tbc;
          fila[i].tsf = atendente[i].ti;
        }
        ...
        {
          tempoAtual += restante[i];
          restante[i] = 0;
          atendente[i].tt = tempoAtual;
...
void simular(int qtdEnt, int ti, int ci, int cf, int ai, int af)
{
  ...
  int quantum;
  printf("\nInforme o valor do Quantum para Round Robin: ");
  scanf("%d", &quantum);
  roundRobin(base, qtdEnt, quantum);
}
